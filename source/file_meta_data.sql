create table &&pldi_user..file_meta_data (
  fmd_id                 number(5, 0),
  ...
  constraint file_meta_data_fad_id_fk foreign key ( fad_id )
    references &&pldi_user..file_adapter_data ( fad_id )
);
-------
comment on column &&pldi_user..file_meta_data.delimiter is
  'The field delimiter (one character only).';
comment on column &&pldi_user..file_meta_data.enclosure is
  'The field enclosure character (one character only).';
-------
create sequence &&pldi_user..file_meta_data_seq 
